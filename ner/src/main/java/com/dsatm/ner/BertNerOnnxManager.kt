// File: com.dsatm.ner/BertNerOnnxManager.kt (Audio Pipeline - REPLACED)
...
class BertNerOnnxManager(private val context: Context) {
      ...
      fun initialize() {
            Log.d(TAG, "Starting initialization of Audio NER manager...")
            try {
                  ...
                  Log.d(TAG, "Audio Post-processor initialized.")

                  Log.i(TAG, "Audio NER manager initialization complete.")
                } catch (e: Exception) {
                  Log.e(TAG, "Failed to initialize Audio NER manager.", e)
                  close()
                  throw e
                }
          }
      ...
      /**
      * Runs the PII detection pipeline on the given text (raw transcript with timestamps).
      */
      fun detectPii(text: String): List<PiiEntity> {
            ...
            // 🛑 Remove Vosk timestamps (e.g., "[1.23-1.50]") before passing to tokenizer
            ...
            try {
                  ...
                  Log.d(TAG, "Audio NER detection complete. Found ${entities.size} entities on clean text.")
                  return entities
                } finally {
